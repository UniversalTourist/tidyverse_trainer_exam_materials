...
We often want to look at our data by some group (if we have) for example country, year, age, gender, education, etc. to spot some differences or similarities (if there is).
...
We are wondering what is the the `mean value of aroma and aftertaste` variables by `country_of_origin` for the `species of Arabica`?
Can we find the true order of that code piece to achieve our goal?
