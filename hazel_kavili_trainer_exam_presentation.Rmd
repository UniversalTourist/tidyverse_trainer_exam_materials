...
Today we'll focus on `dplyr`, a package that helps us to transforms data. 
...
## Some structure of writing dplyr verbs
...
Here is an exmaple. One of the questions on the above was "What is the average total_cup_points for each coffee species?". Let's try to find some answer to that question:
...
We want to look at the mean value of `aroma and aftertaste` variables by `country_of_origin` for the `species of Arabica`. Can we find the true order of that code piece?
